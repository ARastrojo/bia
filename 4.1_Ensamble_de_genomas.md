...
<!--Contar el número de lecturas y comprobar que ambos ficheros tienen el mismo número -->


<!--
**Comprobación de la integridad de los datos descargados**

Es muy importante comprobar la integridad de los datos descargados. El algoritmo [MD5sum](https://en.wikipedia.org/wiki/Md5sum) u otros más modernos ([SHA1sum](https://en.wikipedia.org/wiki/Sha1sum)) ["transforman"](https://en.wikipedia.org/wiki/Cryptographic_hash_function) el contenido de un fichero en una cadena de caracteres denomicado _hash_, que no cambian a menos que el contenido del archivo cambie (el nombre del archivo es irrelevante para esto, sólo importa el contenido). Por lo tanto, si durante la descarga los datos se han corrompido por alguna razón, el hash cambiará. Por ello, es muy frecuente que las bases de datos proporcionen este tipo de hashes para que podamos realizar esta comprobación. 
