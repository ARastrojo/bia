...
Lo instalamos en la base de conda para poder usarlo desde R:
...
<!--
```R
cutadapt <- "/home/usuario/miniconda3/bin/cutadapt"
# cutadapt <-- "/home/usuario/miniconda3/envs/cutadaptenv/bin/cutadapt"
system2(cutadapt, args="--version")
# system(paste0(cutadapt, " --version"))
```
-->
...
#conda install -c bioconda prokka -y # Falló no se porqué
conda install -c bioconda igv -y
conda install -c bioconda blast -y
conda install -c bioconda diamond -y
conda install -c bioconda sra-tools -y
conda install -c bioconda seqkit -y
conda install -c bioconda samtools -y # No creo que esté en las instancias
```

Creo un entorno separado para Lofreq porque necesita python >3.10 y <3.11:
...
También creo un de Prokka por lo mismo, aunque no sé si estará en las instancias:

```bash
conda create -n prokka -c bioconda prokka -y
```

También creo un de lofreq por lo mismo, aunque no sé si estará en las instancias:
...
```
#### Reticulate (to use conda from R)
